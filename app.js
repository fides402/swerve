...
function updateLoadingStep(step, detail) {  const s = document.getElementById('loading-step');  const d = document.getElementById('loading-detail');  if (s) s.textContent = step;  if (d) d.textContent = detail || '';  updateSeedInfo(step);}
...
const AlbumMode = {
  ...
  async open(track) {
    ...
    try {
      ...
      const cover = tidalCover(data.cover, 640) || track.art || '';
      ...
      overlay.innerHTML = `
        <div class="album-modal">
          <button class="album-back" id="album-back-btn">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="15 18 9 12 15 6"/></svg>
            Indietro
          </button>
          <div class="album-header">
            ${cover ? `<img class="album-cover" src="${cover}" alt="">` : '<div class="album-cover-placeholder">💿</div>'}
            <div class="album-meta">
              <div class="album-title">${escHtml(albumTitle)}</div>
              <div class="album-artist">${escHtml(artist)}</div>
              ${year ? `<div class="album-year">${year}</div>` : ''}
              <div class="album-count">${items.length} brani</div>
            </div>
          </div>
          <div class="album-tracklist" id="album-tracklist">
            ${items.map((item, i) => {
              ...
              const isPlaying = t.id && S.currentCardTrack?.tidalId === t.id;
              return `
                <div class="album-track ${isPlaying ? 'playing' : ''}" data-tidal-id="${t.id || ''}" data-idx="${i}">
                  <span class="album-track-num">${t.trackNumber || (i+1)}</span>
                  <div class="album-track-info">
                    <span class="album-track-title">${escHtml(t.title || '?')}</span>
                    <span class="album-track-artist">${escHtml(t.artist?.name || artist)}</span>
                  </div>
                  <span class="album-track-dur">${dur}</span>
                  <button class="album-track-like ${S.myLiked.some(l => l.tidalId === t.id) ? 'liked' : ''}"
                    data-tidal-id="${t.id || ''}" title="Mi piace">
      ...
      overlay.querySelectorAll('.album-track').forEach(row => {
        row.addEventListener('click', e => {
          if (e.target.closest('.album-track-like')) return;
          const tidalId = parseInt(row.dataset.tidalId);
          if (!tidalId) return;
          const item = items.find(it => (it.item?.id || it.id) === tidalId);
          if (!item) return;
          const t = item.item || item;
          const trackObj = {
            tidalId: t.id,
          ...
          // Build queue from this track forward
          const fromHere = items.slice(items.indexOf(item)).map(it => {
            const tr = it.item || it;
            return { tidalId: tr.id, t: tr.title||'?', a: tr.artist?.name||artist,
              al: albumTitle, art: cover, d: (tr.duration||0)*1000,
              isrc: tr.isrc||'', y: year, pop: tr.popularity||0, albumId, source: 'album' };
          });
          Player.setQueue(fromHere, 0);
          overlay.querySelectorAll('.album-track').forEach(r => r.classList.remove('playing'));
          row.classList.add('playing');
        });
      });

      // Like buttons
      overlay.querySelectorAll('.album-track-like').forEach(btn => {
        btn.addEventListener('click', e => {
          e.stopPropagation();
          const tidalId = parseInt(btn.dataset.tidalId);
          if (!tidalId) return;
          const item = items.find(it => (it.item?.id || it.id) === tidalId);
          if (!item) return;
          const t = item.item || item;
          const trackObj = {
            tidalId: t.id, t: t.title||'?', a: t.artist?.name||artist,
            al: albumTitle, art: cover, d: (t.duration||0)*1000,
            isrc: t.isrc||'', y: year, pop: t.popularity||0, albumId, source: 'album'
          };
          const already = S.myLiked.findIndex(l => l.tidalId === t.id);
          if (already >= 0) {
            S.myLiked.splice(already, 1);
            btn.classList.remove('liked');
          } else {
            S.myLiked.push(trackObj);
            btn.classList.add('liked');
            UserProfile.update(trackObj, 'like');
          }
          updateSidebarStats();
          saveState();
        });
